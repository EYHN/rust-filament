...
impl MaterialProvider {
    ...
    pub unsafe fn create_jit_shader_generator(
        engine: &mut crate::filament::Engine,
        optimize_shaders: bool,
    ) -> Option<Self> {
        let generator =
            bindgen::helper_filament_gltfio_material_provider_create_jit_shader_generator(
                engine.native_mut(),
                optimize_shaders,
            );
    ...
    pub unsafe fn create_ubershader_loader(engine: &mut crate::filament::Engine) -> Option<Self> {
        let generator = bindgen::helper_filament_gltfio_material_provider_create_ubershader_loader(
            engine.native_mut(),
        );
        Self::try_from_native(generator)
    }
    ...
    pub unsafe fn get_materials(&self) -> Option<Vec<Material>> {
        let count =
            bindgen::helper_filament_gltfio_material_provider_get_materials_count(self.native());
        let native_material_ptr =
            bindgen::helper_filament_gltfio_material_provider_get_materials(self.native());
    ...
    pub unsafe fn destroy_materials(&mut self) {
        bindgen::helper_filament_gltfio_material_provider_destroy_materials(self.native_mut())
    }

    #[inline]
    pub unsafe fn needs_dummy_data(&mut self, attribute: VertexAttribute) -> bool {
        bindgen::helper_filament_gltfio_material_provider_needs_dummy_data(
            self.native(),
            attribute.into(),
        )
    }
...
impl Drop for MaterialProvider {
    fn drop(&mut self) {
        unsafe { bindgen::helper_filament_gltfio_material_provider_delete(self.native_mut()) }
    }
}
